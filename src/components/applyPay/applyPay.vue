<template>
  <transition name="slide">
    <div class="g-course-pay">
      <scroll :data="applyInfo" ref="scroll">
        <div class="g-scroll-continer">
          ...
          <div class="chunk info-wrapper">
            ...
            <div class="info">
              <apply-item v-if="applyInfo" :apply-result-base="applyInfo.activity"
                          :apply-result-detail="applyInfo.ticket" :apply-result="applyInfo"
      ...
      <bottom-btn v-if="applyInfo" :price="applyInfo.activity.price" :btnstr="'立即支付'" @confirm="toPay"></bottom-btn>
    ...
    methods: {
      ...
      wxPayH5 (data) {
        var url = data.mweb_url,
          redirectUri = encodeURIComponent(`${util.config.payRedirectUri}?id=${this.applyTargetID}&applyId=${this.applyID}&payType=weixin&payTargetType=activity`);
      ...
      _getAlipayConfig () {
        ...
        var url = `${util.config.payRedirectUri}?id=${this.applyTargetID}&applyId=${this.applyID}&payType=alipay&payTargetType=activity`;
        let params = {
          id: this.applyInfo.order.id,
